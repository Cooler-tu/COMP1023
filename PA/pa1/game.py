...
def init_gameboard():
    ...
    """
    ### TASK 1 STARTS HERE ###
    table = [[int(0)]*6 for i in range(20)]
    return table  # this line also need to change
    ### TASK 1 ENDS HERE ###
...
def default_random_number_generator():
    ...
    index1 = random.randint(0, len(candidates) - 3)  # Generate random number from 0 to len(candidates) - 1      
    index2 = random.randint(0, len(candidates) - 3)
...
def is_valid_move(game_board, current_shape, current_location, current_rotation):
    ...
    ### TASK 3 STARTS HERE ###
    for i in range(4): # range 0 to 3
        now_x = current_location[0] + shapes[current_shape][current_rotation][i][0]
        now_y = current_location[1] + shapes[current_shape][current_rotation][i][1]
        if (now_x < 0 or now_y < 0 or now_x > 19 or now_y > 5):
            return False
        if (game_board[now_x][now_y] != 0):
            return False
    return True
    ### TASK 3 ENDS HERE ###
        ...
        for j in range(0, 6):
            ...
            if game_board[i][j] == 0:
                ...
                for k in range(i-1, -1, -1):
                    if game_board[k][j] > 0:
                        game_board[i-x][j] = game_board[k][j]
                        x += 1
                for p in range(0, i-x):
                    game_board[p][j] = 0
                vis[k] = 1
