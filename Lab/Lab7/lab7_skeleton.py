...
def check_out_book(username, title):
    ...
    if title in books:
        ...
        if availability[index]:
            ...
            index_name = users.index(username)
            user_borrow_books[index_name].append(title)
    ...
    if title in books:
        ...
        if not availability[index]:
            if borrowers[index] == username:
                ...
                borrowers.remove(username)
                index_name = users.index(username)
                user_borrow_books[index_name].remove(title)
                if user_borrow_books[index_name] is None:
                    user_borrow_books.pop(index_name)
                    users.pop(index_name)
...
def search_books(keyword, check_avail):
    ...
    for index in range(len(books)):
        ...
        if keyword in title.lower():
            if (not check_avail) or (check_avail and availability[index]):
                print(f"- {title} by {authors[index]}")
                tmp = True
    if not tmp:
        print(f"(No books found with the given keyword)")
