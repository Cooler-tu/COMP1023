...
def task_1_add_course(schedule, code, slots_to_add):
    ...
    all_slots_free = True
    for i in slots_to_add:
        if schedule[i[0]][i[1]] != None:
...
def task_3_view_slice(schedule, start_day, end_day):
    ...
    if 0 <= start_day <= end_day <= 4:
        for i in schedule[start_day:end_day+1]:
            display_row = []
            for x in i:
                if x is None:
                    display_row.append("---")
            ...
            result_text += str(day_names[day]) + ": " + str(display_row) + "\n"
            #不能用index因为如果有很多天是空的，list查找后index的返回值并不会是想要的那天
            day += 1
